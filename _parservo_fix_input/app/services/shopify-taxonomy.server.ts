...
export function detectShopifyCategoryPath(product: ImportedProductForCategory) {
  ...
  if (/\b(lunarove|sneakers?|trainers?|sports shoes?|кросівк)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.SNEAKERS;
  if (/\b(ankle boots?|ботильйон)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.ANKLE_BOOTS;
  if (/\b(boots?|черевик)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.BOOTS;
  if (/\b(sandals?|сандал)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.SANDALS;
  if (/\b(loafers?|лофер)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.LOAFERS;
  if (/\b(pumps?|heeled shoes?|heels?|туфл)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.HEELS;
  if (/\b(flip flops?|slides?|slippers?|шльопанц)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.FLIP_FLOPS;

  // Clothing tops.
  if (/\b(polo|поло)\b/i.test(text)) return SHOPIFY_CATEGORY_PATHS.POLOS;
  if (/\b(t\s?shirt|tee\b|long sleeve|long-sleeve|футболк)/i.test(text)) return SHOPIFY_CATEGORY_PATHS.T_SHIRTS;
  if (/\b(hoodie|худі)\b/i.test(text)) return SHOPIFY_CATEGORY_PATHS.HOODIES;
  if (/\b(sweatshirt|світшот)\b/i.test(text)) return SHOPIFY_CATEGORY_PATHS.SWEATSHIRTS;
  if (/\b(sweater|pullover|knitwear|светр)\b/i.test(text)) return SHOPIFY_CATEGORY_PATHS.SWEATERS;
  if (/\b(shirt|сорочк)\b/i.test(text)) return SHOPIFY_CATEGORY_PATHS.SHIRTS;
