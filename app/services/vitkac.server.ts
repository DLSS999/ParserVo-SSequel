...
const CATEGORY_UA: Array<[RegExp, string]> = [
  // Important: use word boundaries so "sweatshirt" is not detected as "t-shirt"
  // and "t-shirt" is not detected as a plain "shirt".
  [/\bt[-\s]?shirts?\b|\btees?\b|\blong[-\s]?sleeves?\b/i, "Футболка"],
  [/\bpolos?\b/i, "Поло"],
  [/\bhoodies?\b/i, "Худі"],
  [/\bsweatshirts?\b|\bsweaters?\b/i, "Світшот"],
  [/\bpumps?\b|\bheeled shoes?\b|\bheels?\b|\bhigh[-\s]?heeled\b/i, "Туфлі на підборах"],
  [/\bslides?\b|\bflip[-\s]?flops?\b|\bslippers?\b/i, "Шльопанці"],
  [/\bsports shoes?\b|\bsneakers?\b|\btrainers?\b/i, "Кросівки"],
  [/\bsandals?\b/i, "Сандалі"],
  [/\bankle boots?\b/i, "Ботильйони"],
  [/\bboots?\b/i, "Черевики"],
  [/\bloafers?\b/i, "Лофери"],
  [/\bbags?\b|\bhandbags?\b|\bshoulder bags?\b|\btote bags?\b/i, "Сумка"],
  [/\bdresses?\b/i, "Сукня"],
  [/\bshirts?\b/i, "Сорочка"],
  [/\bjackets?\b|\bbomber jackets?\b/i, "Куртка"],
  [/\bcoats?\b/i, "Пальто"],
  [/\bjeans\b/i, "Джинси"],
  [/\bshorts\b/i, "Шорти"],
  [/\btrousers?\b|\bpants\b/i, "Штани"],
  [/\bskirts?\b/i, "Спідниця"],
];

const ALPHA_SIZE_ORDER = ["XXXXS", "XXXS", "XXS", "XS", "S", "M", "L", "XL", "XXL", "XXXL", "XXXXL"];
const ONE_SIZE_LABELS = ["UNI", "ONE SIZE", "ONESIZE", "OS", "U"];

function isOneSizeLabel(value: string | null | undefined) {
  ...
  return ONE_SIZE_LABELS.includes(normalized);
}
...
function toNumber(value: string | number | undefined | null) {
  ...
  if (lastComma >= 0 && lastDot >= 0) {
    // Supports both European and international Vitkac formats:
    // 2.732,00 / 2,732.00 -> 2732.00
    ...
    if (parts.length > 2) {
      ...
      // 115,92 / 115.92 -> decimal separator.
      normalized = parts[0] + "." + tail;
    } else if (tail.length === 3 && parts[0].length <= 3) {
      // 2,732 / 2.732 -> thousand separator.
      normalized = parts.join("");
    } else {
  ...
  const parsed = Number(sign + normalized.replace(/[^\d.]/g, ""));
...
function isBadPriceLine(line: string) {
  ...
  // Vitkac pages contain payment/installment amounts near the product block.
  // After HTML stripping those blocks can look like a normal price and were
  // previously imported as the supplier price, e.g. 179,04 zł / 415,04 zł.
  if (/installments|instalments|oblicz rat|rat[ayęe]|raty|rata|rataln|płatno|platno|paypo|klarna|twisto|przelewy24|credit card|zapłać teraz|zaplac teraz|pay now|shipping|delivery|dostawa|zwrot|return|gift card|karta podarunkowa|pobierz|aplikacji/.test(normalized)) {
    return true;
  }

  // Examples: "4 x 415,04 zł", "4x415,04zł", "5 × 179,04 zł".
  if (/\b\d{1,2}\s*[x×]\s*\d/.test(normalized) || /\b\d{1,2}[x×]\d/.test(compact)) {
    return true;
  }
...
function centsPart(value: number) {
  return Math.round((Math.abs(value) % 1) * 100);
}

function isLikelyInstallmentAmount(candidate: PriceCandidate, maxCandidateValue: number) {
  // Installment artifacts on Vitkac often end with ,04 zł and are much lower
  // than the real product price visible in the same product window. Keep them
  // only if there is no higher product-like amount to compare against.
  return centsPart(candidate.value) === 4 && maxCandidateValue > candidate.value * 1.6;
}

function chooseProductPriceCandidate(pricePool: PriceCandidate[]) {
  // After installment/service values are filtered out, Vitkac current sale price
  // is the lowest product-like amount in the product price block. The higher
  // nearby amount is usually the old crossed-out price.
  return [...pricePool].sort((a, b) => a.value - b.value || a.index - b.index)[0];
}
...
function extractPrice(lines: string[], jsonProduct: ExtractedJsonProduct, symbol: string | null, supplierUrl: string) {
  ...
  // Drop tiny helper values and installment-like artifacts. The previous logic
  // selected the smallest value from the window, which incorrectly treated
  // installment amounts such as 179,04 zł / 415,04 zł as the product price.
  const productLikeCandidates = candidates.filter((candidate) => {
    if (candidate.value < maxCandidateValue * 0.45) return false;
    if (isLikelyInstallmentAmount(candidate, maxCandidateValue)) return false;
    return true;
  });
  const pricePool = productLikeCandidates.length > 0 ? productLikeCandidates : candidates;
  const priceCandidate = chooseProductPriceCandidate(pricePool);
  ...
  for (const [pattern, translated] of CATEGORY_UA) {
    if (pattern.test(source)) {
      return {
        ...
        productType: categoryEnglishFromUa(translated),
      };
    }
  }
...
function extractVariants(lines: string[], html: string) {
  ...
  function addVariant(rawSize: string, available: boolean) {
    const cleanSize = cleanText(rawSize).trim();
    if (!cleanSize) return;

    const oneSize = isOneSizeLabel(cleanSize);
    const supplierSizeLabel = oneSize ? "UNI" : cleanSize.replace(".", ",").trim();
    const normalizedSize = oneSize ? "Default Title" : supplierSizeLabel.replace(",", ".");

    if (!oneSize && !/^\d{1,3}(?:[,.]\d)?$|^[A-Z]{1,5}$/i.test(supplierSizeLabel)) return;
  ...
  const text = lines.join("\n");
  const sizeToken = "\\d{1,3}(?:[,.]5)?|XXXS|XXS|XS|S|M|L|XL|XXL|XXXL|UNI|ONE\\s+SIZE|ONESIZE|OS";
  const sizeStatusRegex = new RegExp(
    `\\b(${sizeToken})\\b\\s*(LAST ITEM|OSTATNIA SZTUKA|AVAILABLE|DOSTĘPNY|DOSTEPNY|IN STOCK|POWIADOM O DOSTĘPNOŚCI|POWIADOM O DOSTEPNOSCI|NOTIFY|SOLD OUT|NIEDOSTĘPNY|NIEDOSTEPNY)`,
    "gi",
  );
  let match: RegExpExecArray | null;
  ...
  while ((match = optionRegex.exec(html)) !== null) {
    ...
    const optionMatch = optionText.match(new RegExp(`^(${sizeToken})\\b`, "i"));
    if (optionMatch?.[1]) {
  ...
  // Vitkac bags/accessories often use UNI / ONE SIZE. This is not a real customer-facing size.
  // Store it internally as the default Shopify variant so the product has stock, but do not create a Size option.
  if (variants.length === 0 && /\b(UNI|ONE\s+SIZE|ONESIZE)\b/i.test(text)) {
    addVariant("UNI", !/POWIADOM|NOTIFY|SOLD|NIEDOST/i.test(text));
  }

  if (variants.length === 0) {
    // Fallback for pages where Vitkac renders the size dropdown only client-side.
    // Do not run this fallback for one-size pages because it can pick random numbers/sizes
    // from recommendations, descriptions or hidden page data.
