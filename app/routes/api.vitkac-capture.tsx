...
function isDefaultNoSizeVariant(variant: { size?: string | null; supplierSizeLabel?: string | null }) {
  ...
  return ["DEFAULT TITLE", "UNI", "ONE SIZE", "ONESIZE", "OS"].includes(size)
    || ["DEFAULT TITLE", "UNI", "ONE SIZE", "ONESIZE", "OS"].includes(supplierSize);
}
...
function hasAvailableSupplierStock(variants: Array<{ available: boolean }>) {
  return variants.some((variant) => variant.available);
}

function buildVariantSku(symbol: string | null | undefined, productId: string, variant: { size?: string | null; supplierSizeLabel?: string | null }) {
  const base = symbol || productId;
  return isDefaultNoSizeVariant(variant) ? base : `${base}-${variant.size || variant.supplierSizeLabel}`;
}
...
async function updateExistingProductFromCapture(args: {
    ...
    if (matchedVariant) {
      ...
      await db.importedVariant.create({
        data: {
          ...
          available: parsedVariant.available,
          lastAvailable: false,
          sku: buildVariantSku(parsedProduct.supplierSymbol, parsedProduct.supplierProductId, parsedVariant),
          price: pricing.salePriceUah,
          compareAtPrice: pricing.compareAtPriceUah || 0,
        },
      });
  ...
  const stockSourceStatus = hasAvailableSupplierStock(refreshedVariants) ? "supplier_available" : "supplier_sold_out";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    const stockSourceStatus = hasAvailableSupplierStock(parsedProduct.variants) ? "supplier_available" : "supplier_sold_out";
    ...
    await db.importedVariant.createMany({
      data: sortVariants(parsedProduct.variants).map((variant: any) => ({
        ...
        lastAvailable: variant.available,
        sku: buildVariantSku(parsedProduct.supplierSymbol, parsedProduct.supplierProductId, variant),
        price: pricing.salePriceUah,
        compareAtPrice: pricing.compareAtPriceUah || 0,
      })),
    });
